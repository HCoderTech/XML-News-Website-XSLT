...
<xsl:template match="/">
    ...
    <xsl:for-each select = "/nodes/results/node">
	 <xsl:if test="not(contains(translate(title, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'),translate($search, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')))=true">
				<div class="inews">
				<table class="table">
